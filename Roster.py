...
# --- 3. 手写稿初始排班数据 (锁定 image_6a7ddc.png) ---
...
if week_key not in st.session_state.cloud_db:
    if week_key == "2026-02-09": # 强制加载识别出的手写排班
...
col_cfg = {}
for d in ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]:
    col_cfg[f"{d}_起"] = st.column_config.SelectboxColumn(f"{d}|起", options=TIME_OPTIONS)
    col_cfg[f"{d}_止"] = st.column_config.SelectboxColumn(f"{d}|止", options=TIME_OPTIONS)

t_h = (len(current_df) + 1) * 35 + 60
edited_df = st.data_editor(current_df, column_config=col_cfg, use_container_width=True, hide_index=True, height=t_h, disabled=is_readonly, key=f"vF_{week_key}")
st.session_state.cloud_db[week_key] = edited_df

if not is_readonly:
    if st.button(f"💾 永久同步 {week_key} 数据", use_container_width=True):
        st.session_state.cloud_db[week_key] = edited_df
        st.success("✅ 同步成功！")
...
if st.session_state.role == "owner":
    ...
    for _, row in edited_df.iterrows():
        ...
        settle_list.append({"员工": name, "周工时(h)": p_h, "工资": f"${round(p_w, 2)}", "支付": p_type})
    ...
    m1.metric("现金结算 (Cash)", f"${round(t_cash, 2)}")
